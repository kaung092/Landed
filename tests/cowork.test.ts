...
test("a rejection never inserts a new posting when the company has one — it raises a confirm (pending match), then resolving it applies the rejection", () => {
  // One Percepta posting in the interview stage, stored title "Senior Platform Engineer".
  const id = seedApp({ company: "Percepta", role: "Senior Platform Engineer", status: "interview", interviewed: true });
  ...
  submitJobResult({
    ...
    records: [{ company: "Percepta", role: "Software Engineer", status: "rejected" }],
  });
  ...
  const pending = listPendingMatches().filter((m) => m.companyName === "Percepta");
